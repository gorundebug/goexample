...
- **Metrics** (Prometheus): `GET http://localhost:9093/metrics`
- **Status** (topology visualization): `GET http://localhost:9093/status`
...
The application listens on `9093` for HTTP and
`9203` for gRPC by default. Set
`ANALYTICS_SERVICE_HTTP_PORT` or
`ANALYTICS_SERVICE_GRPC_PORT` to change the listener and
container-side mapping. Set `ANALYTICS_SERVICE_HOST_HTTP_PORT`
or `ANALYTICS_SERVICE_HOST_GRPC_PORT` to change only the
forwarded host port.
