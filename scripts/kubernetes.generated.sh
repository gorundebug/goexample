...
NAMESPACE="${KUBERNETES_NAMESPACE:-goexample}"
...
deploy_services() {
  ...
  helm upgrade --install analyticsservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/goexample/analyticsservice" \
  ...
  helm upgrade --install automationservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/goexample/automationservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install inventoryservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/goexample/inventoryservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install orderservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/goexample/orderservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
